...
impl LeanIMT {
    ...
    fn rebuild_cache_from_leaves(&mut self) {
        // Populate subtree_cache with zero-subtree hashes so that
        // compute_node_at_level_scalar can fall back to them for empty regions
        // without exponential recursion.
        let zero_scalar = BlsScalar::from_u256(U256::from_u32(&self.env, 0));
        let mut sponge = PoseidonSponge::<3, BlsScalar>::new(&self.env);
        let mut current = zero_scalar;
        self.subtree_cache = Map::new(&self.env);
        self.cache_subtree_level(0, current.clone());
        for level in 1..=self.depth {
            current = self.hash_pair_with_sponge(&mut sponge, current.clone(), current.clone());
            self.cache_subtree_level(level, current.clone());
        }

        // Rebuild sparse_cache bottom-up from stored leaves.
        //
        // The previous implementation cleared sparse_cache here but never rebuilt it.
        // That caused incremental_update (called during every deposit after the first)
        // to read wrong sibling values from subtree_cache (all-zeros) instead of the
        // actual computed siblings, producing incorrect roots from deposit #3 onward.
        self.sparse_cache = Map::new(&self.env);

        let leaf_count = self.leaves.len() as u32;
        if leaf_count == 0 {
            self.root = bls_scalar_to_bytes(current);
            return;
        }

        // Cache all leaf scalars at level 0.
        for leaf_idx in 0..leaf_count {
            let leaf_bytes = self.leaves.get(leaf_idx).unwrap();
            let leaf_scalar = bytes_to_bls_scalar(&leaf_bytes);
            self.cache_sparse_node(0, leaf_idx, leaf_scalar);
        }

        // Build internal nodes level by level.  At each level, only the
        // ceil(active / 2) leftmost parents are non-empty; siblings in the
        // empty half fall back to subtree_cache automatically via
        // compute_node_at_level_scalar.
        let mut active_count = leaf_count;
        for level in 0..self.depth {
            let parent_count = (active_count + 1) / 2;
            for parent_idx in 0..parent_count {
                let left_idx = parent_idx * 2;
                let right_idx = left_idx + 1;
                let left_scalar = self.compute_node_at_level_scalar(left_idx, level);
                let right_scalar = self.compute_node_at_level_scalar(right_idx, level);
                let parent_scalar =
                    self.hash_pair_with_sponge(&mut sponge, left_scalar, right_scalar);
                self.cache_sparse_node(level + 1, parent_idx, parent_scalar);
            }
            active_count = parent_count;
        }

        // Recompute root from the rebuilt cache, overriding any stale stored root.
        if let Some(root_scalar) = self.sparse_cache.get((self.depth, 0u32)) {
            self.root = bls_scalar_to_bytes(root_scalar);
        }
    }
